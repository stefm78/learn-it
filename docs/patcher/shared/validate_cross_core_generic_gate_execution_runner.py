...
def main() -> int:
    report_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("docs/registry/reports/cross_core_generic_gate_execution_runner_validation.yaml")
    findings = []
    checked = []
    combined_parts = []
    ...
    for marker in REQUIRED_MARKERS:
        if marker not in combined:
            findings.append({"finding_id": "missing_required_marker", "severity": "blocking", "path": "combined_phase58_surface", "message": marker})

    status = "PASS" if not findings else "FAIL"
    lines = [
        "cross_core_generic_gate_execution_runner_validation:",
        "  schema_version: '0.1'",
    ...
    lines.extend([
        ...
        "    generic_gate_execution_runner_defined: true",
        "    smoke_blocks_before_downstream_gates: true",
        "    core_mutation_authorized: false",
        "    backlog_closure_authorized: false",
        "    release_or_promotion_authorized: false",
    ])
