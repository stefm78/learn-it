...
def apply_target_version(doc: Dict[str, Any], role: str, target_versions: Dict[str, Any]) -> None:
    ...
    if not isinstance(entry, dict):
        raise MaterializationError(f"target_versions.{role} doit être un objet")
...
def build_release_notes(release_id: str, release_path: str, plan: Dict[str, Any], manifest: Dict[str, Any]) -> str:
    ...
    for entry in cores:
        if isinstance(entry, dict):
            lines.append(
                f"- `{entry.get('role')}` → `{entry.get('core_id')}` / `{entry.get('version')}`"
            )
...
def build_report(status: str, release_id: str, release_path: str, files_written: List[str], notes: List[str]) -> Dict[str, Any]:
    return {
        "RELEASE_MATERIALIZATION_REPORT": {
            ...
            "manifest_written": any(path.endswith("manifest.yaml") for path in files_written),
            "notes": notes,
        }
    }
...
def main() -> None:
    if len(sys.argv) < 3:
        raise SystemExit(
            "Usage: materialize_release.py <release_plan.yaml> <patched_dir> [report.yaml] [release_candidate_notes.md]"
        )
    ...
    try:
        ...
        replacements = collect_replacements(plan)
        role_docs: Dict[str, Dict[str, Any]] = {}
        ...
        notes_content = build_release_notes(release_id, str(release_path), plan, manifest)
        write_text(notes_path, notes_content)

        report_notes = [
            f"release matérialisée sous {release_path}",
            "aucune modification de docs/cores/current/",
        ]
        dump_yaml(report_path, build_report("PASS", release_id, str(release_path), files_written, report_notes))
        ...
        dump_yaml(
            report_path,
            build_report("FAIL", release_id, release_path, [], [str(exc)]),
        )
