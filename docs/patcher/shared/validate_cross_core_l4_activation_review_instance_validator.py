...
REPO = Path.cwd()
FILES = ['docs/patcher/shared/validate_cross_core_l4_activation_review_instance.py', 'docs/registry/reports/l4_activation_review_template_instance_validation.yaml', 'docs/pipelines/cross_core_contract/L4_ACTIVATION_REVIEW_INSTANCE_VALIDATION.md', 'docs/pipelines/cross_core_contract/validators/l4_activation_review_instance_validation.yaml', 'docs/pipelines/cross_core_contract/L4_ACTIVATION_REVIEW_INPUT.md', 'docs/pipelines/cross_core_contract/schemas/l4_activation_review.schema.yaml', 'docs/pipelines/cross_core_contract/templates/l4_activation_review.template.yaml', 'docs/pipelines/cross_core_contract/AI_PROTOCOL.yaml', 'docs/pipelines/cross_core_contract/pipeline.md', 'docs/pipelines/cross_core_contract/state.yaml', 'docs/pipelines/cross_core_contract/l4_transition_checklist.yaml', 'docs/pipelines/cross_core_contract/requests/CCR_PATCH_LIFECYCLE_ESCALATION_THRESHOLD_N_R01.yaml']
REQUIRED_MARKERS = [
...
def main() -> int:
    report_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("docs/registry/reports/cross_core_l4_activation_review_instance_validator_validation.yaml")
    findings = []
    checked = []
    combined_parts = []
    for rp in FILES:
    ...
    for marker in REQUIRED_MARKERS:
        if marker not in combined:
            findings.append({'finding_id': 'missing_required_marker', 'severity': 'blocking', 'path': 'combined_phase51_surface', 'message': marker})
    ...
    lines = [
        'cross_core_l4_activation_review_instance_validator_validation:',
        "  schema_version: '0.1'",
