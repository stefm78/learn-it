...
def get_meta(root: Dict[str, Any]) -> Dict[str, Any]:
    return {k: v for k, v in root.items() if k not in SECTIONS}
...
def validate_references(root: Dict[str, Any]) -> None:
    ids = collect_all_ids(root)
    for _, item in iter_all_objects(root):
        deps = item.get("depends_on", [])
        if isinstance(deps, list):
            for dep in deps:
                if isinstance(dep, str) and not (dep in ids or dep.startswith("REF_CORE_") or dep.startswith("LINK_REF_CORE_")):
                    raise PatchError(f"Dépendance orpheline: {item.get('id')} -> {dep}")
        ...
        if isinstance(rels, list):
            for rel in rels:
                ...
                if isinstance(tgt, dict):
                    tgt_id = tgt.get("id")
                    if isinstance(tgt_id, str) and not (tgt_id in ids or tgt_id.startswith("REF_CORE_") or tgt_id.startswith("LINK_REF_CORE_")):
                        raise PatchError(f"Relation orpheline: {item.get('id')} -> {tgt_id}")
        ...
        if isinstance(rels, list):
            for rel in rels:
                if isinstance(rel, dict):
                    tgt = rel.get("target")
                    if isinstance(tgt, dict) and tgt.get("id") == old_id:
                        tgt["id"] = new_id
                        rewritten += 1
    return rewritten
...
def main() -> None:
    if len(sys.argv) < 4:
        raise SystemExit("Usage: apply_patch_v4.py <patchfile> <rootpath> <whatif:true|false> [report_path]")
    ...
    prepared: List[Tuple[str, Any, Any, Dict[str, Any], str | None]] = []
    ...
    try:
        for file_entry in files:
            ...
            for op in file_entry.get("operations", []):
                ...
                if op_type == "assert":
                    ...
                    value = op.get("value", {})
                    file_report["operations"].append(f"note {value.get('id', 'no-id')}")
            ...
            prepared.append((full_path, original, doc, file_report, expected_core_type))
        ...
        if whatif:
            ...
            for full_path, original, doc, _, _ in prepared:
