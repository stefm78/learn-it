...
REQUIRED = {
    'docs/pipelines/cross_core_contract/validators/validate_constitution_referentiel_link_reconstruction.contract.yaml': [
        ...
        'constitution_reconstruction_PASS', 'referentiel_reconstruction_PASS', 'link_reconstruction_PASS',
        'cross_core_reference_ids_resolvable', 'no_orphan_inter_core_reference', 'no_duplicate_canonical_id_across_cores',
        'version_binding_declared', 'release_candidate_reconstructs_all_impacted_cores',
        'core_mutation_authorized: false', 'backlog_closure_authorized: false', 'release_or_promotion_authorized: false',
    ],
    'docs/pipelines/cross_core_contract/validators/validate_constitution_referentiel_link_reconstruction.contract.md': [
        'validate_constitution_referentiel_link_reconstruction', 'contract_defined: true', 'l4_active_now: false', 'executable_as_l4_gate_now: false',
    ],
    'docs/pipelines/cross_core_contract/AI_PROTOCOL.yaml': [
        'validate_constitution_referentiel_link_reconstruction_contract:', 'contract_defined: true', 'executable_as_l4_gate_now: false',
    ],
    'docs/pipelines/cross_core_contract/pipeline.md': ['validate_constitution_referentiel_link_reconstruction contract', 'executable_as_l4_gate_now: false'],
    'docs/pipelines/cross_core_contract/state.yaml': ['validate_constitution_referentiel_link_reconstruction_contract_defined: true', 'executable_as_l4_gate_now: false'],
...
def main():
    report_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path('docs/registry/reports/constitution_referentiel_link_reconstruction_validator_contract_validation.yaml')
    findings = []
    checked = []
    for rp in FILES:
        p = REPO / rp
    ...
    lines = ['constitution_referentiel_link_reconstruction_validator_contract_validation:', "  schema_version: '0.1'", f"  generated_at: '{iso_now()}'", f'  status: {status}', '  validator_id: validate_constitution_referentiel_link_reconstruction', '  contract_defined: true', '  l4_active_now: false', '  executable_as_l4_gate_now: false', '  checked_files:']
    for item in checked:
