...
REQUIRED_MARKERS = [
    ...
    "status: BLOCKED_DOWNSTREAM_GATE_INPUTS_NOT_MATERIALIZED",
...
FORBIDDEN_MARKERS = [
    ...
    "backlog_closure_requested: true",
]
...
def main() -> int:
    report_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("docs/registry/reports/cross_core_generic_authorized_dry_run_gate_smoke_validation.yaml")
    findings = []
    checked = []
    combined_parts = []
    ...
    for marker in REQUIRED_MARKERS:
        if marker not in combined:
            findings.append({"finding_id": "missing_required_marker", "severity": "blocking", "path": "combined_phase59_surface", "message": marker})

    status = "PASS" if not findings else "FAIL"
    lines = [
        "cross_core_generic_authorized_dry_run_gate_smoke_validation:",
        "  schema_version: '0.1'",
    ...
    lines.extend([
        ...
        "    authorized_dry_run_contract_shape_passes: true",
        "    smoke_reaches_downstream_gate_input_boundary: true",
        "    downstream_gates_executed_on_smoke: false",
        "    core_mutation_authorized: false",
        "    backlog_closure_authorized: false",
        "    release_or_promotion_authorized: false",
