# App Factory — Plan de cadrage et d’implantation
...
### 2.2. Introduction d’une couche App Factory

La fabrique applicative doit être gouvernée par deux nouveaux artefacts canoniques distincts :

1. `docs/cores/current/app_factory_architecture.yaml`
2. `docs/cores/current/app_factory_state.yaml`

### 2.3. Rôle des deux artefacts

#### A. `app_factory_architecture.yaml`
...
Il est **déconseillé** de fusionner ces deux rôles dans un seul `app_factory.yaml`.
...
- `app_factory_architecture.yaml`
- `app_factory_state.yaml`

### 3.2. Un pipeline explicite de gouvernance
Sous `docs/pipelines/app_factory/` :
- `pipeline.md`
- `state.yaml`
- `inputs/`
- `work/`
...
- `Make20AppFactoryAudit.md`
- `Make21AppFactoryArbitrage.md`
- `Make22AppFactoryPatch.md`
- `Make23AppFactoryValidation.md`
- `Make24AppFactoryReview.md`
...
## 4. Positionnement de la couche App Factory

La couche App Factory ne gouverne pas directement :
...
> Le pipeline `app_factory` ne gouverne pas une application produite.  
...
- confirmer le nom du pipeline cible : `app_factory` ;
...
#### Pour `app_factory_architecture.yaml`
...
## PHASE 2 — Définition du pipeline `app_factory`
...
Outputs proposés :
- `work/01_scan/app_factory_inventory.md`
...
Outputs proposés :
- `work/02_audit/app_factory_audit.md`
- `reports/app_factory_scorecard.yaml`
...
Outputs proposés :
- `work/03_arbitrage/app_factory_arbitrage.md`

#### STAGE_04_FACTORY_PATCH_SYNTHESIS
But :
- synthétiser les évolutions retenues sur les artefacts App Factory ;
- produire un patch destiné à l’architecture, à l’état, ou aux deux.

Outputs proposés :
- `work/04_patch/app_factory_patchset.yaml`
...
Outputs proposés :
- `work/05_validation/app_factory_patch_validation.yaml`
- `reports/app_factory_patch_validation_report.md`
...
Outputs proposés :
- `work/06_apply/patched/app_factory_architecture.yaml`
- `work/06_apply/patched/app_factory_state.yaml`
- `reports/app_factory_execution_report.yaml`

#### STAGE_07_FACTORY_CORE_VALIDATION
But :
- valider la cohérence d’ensemble des artefacts App Factory patchés ;
- vérifier leur lisibilité canonique ;
- vérifier leur cohérence avec la Constitution et le Référentiel.

Outputs proposés :
- `work/07_validation/app_factory_core_validation.yaml`
- `reports/app_factory_core_validation_report.md`
...
Outputs proposés :
- `reports/app_factory_promotion_report.yaml`
...
Outputs proposés :
- `reports/app_factory_closeout_report.yaml`
- `outputs/app_factory_summary.md`
...
### Scorecard proposée
Le fichier `app_factory_scorecard.yaml` doit permettre de suivre dans le temps au moins :
...
### Objectif
Inscrire la couche App Factory dans la gouvernance standard du repo.
...
Créer un seul `app_factory.yaml` qui mélange :
- cible ;
- constat ;
- backlog ;
- audit ;
...
Créer un pipeline `app_factory` qui produit seulement des rapports et aucune vérité canonique stabilisée.

### Risque 3 — Glissement de couche
Utiliser `app_factory` pour réécrire indirectement la Constitution ou le Référentiel au lieu de gouverner la capacité de production.
...
- [ ] le nom `app_factory` est accepté ;
...
3. définition de la structure YAML de `app_factory_architecture.yaml` ;
4. définition de la structure YAML de `app_factory_state.yaml` ;
5. rédaction du `pipeline.md` de `docs/pipelines/app_factory/` ;
6. rédaction du prompt `Make20AppFactoryAudit.md` ;
...
- ne pas créer un seul `app_factory.yaml` ;
- créer une couche App Factory explicite ;
...
> App Factory Architecture = HOW  
> App Factory State = WHERE WE ARE NOW
