...
Intent:
- discover published pipelines from docs/registry/pipelines.md
- inspect lightweight run/scope state when available
- recommend continue vs new run
- build a short deterministic AI launch prompt

Bootstrap scope:
- generic discovery from registry
- constitution-specific run/scope recommendation
...
def discover_constitution(repo_root: Path) -> dict[str, Any]:
    ...
    if len(active_runs) == 1:
        run = active_runs[0]
        result["recommended_action"] = "continue_or_open_new"
...
def build_constitution_prompt(branch: str, state: dict[str, Any]) -> str:
    ...
    if action == "continue_or_open_new":
        ...
        others_suffix = f" Autres scopes publiés disponibles pour un nouveau run: {others}." if others else ""
        return (
            ...
            f"Propose explicitement soit de continuer ce run, soit d'ouvrir un nouveau run sur un autre scope publié, puis arrête-toi.{others_suffix}"
        )
...
def main() -> int:
    ...
    if args.pipeline != "constitution":
        ...
        print(yaml.safe_dump({
            ...
            "status": "unsupported_in_bootstrap_v2",
