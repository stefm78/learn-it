...
def main() -> None:
    if len(sys.argv) < 4:
        raise SystemExit(
            "Usage: apply_learnit_core_patch_v2.py <patchfile> <rootpath> <whatif:true|false>"
        )
    ...
    for file_entry in files:
        ...
        for op in operations:
            op_type = op["op"]
            if op_type == "assert":
                op_logs.append(apply_assert(root, op))
            elif op_type == "append":
                ...
                op_logs.append(apply_replace(root, op))
                changed = True
            elif op_type == "replace_field":
                op_logs.append(apply_replace_field(root, op))
                changed = True
            elif op_type == "remove":
                op_logs.append(apply_remove(root, op))
                changed = True
            elif op_type == "rename":
                op_logs.append(apply_rename(root, op))
                changed = True
    ...
    if atomic:
        for staged in staged_docs:
            # already validated individually; this block is kept for future multi-file cross-checks
            _ = staged
