...
- `docs/cores/current/`
- `docs/cores/releases/`
- `legacy/` and any preserved pre-framework materials located there
...
Non-authoritative materials may inform analysis, migration, or traceability,
but they do not define the current system state.
...
If an artifact can be mistaken for canonical while not being canonical,
it must be renamed, moved, marked, or archived.
...
> A release changes what is authoritative.

It must therefore be explicit, reviewable, and reproducible.
...
This repository can be operated by one person or many,
but the roles should still be conceptually separated.
...
but they should not be confused conceptually.
